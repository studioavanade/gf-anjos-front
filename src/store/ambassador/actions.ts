...
AmbassadorService().getAmbassador(ambassador).then((res: any) => {
  const getAmb = {
    ...ambassador, 
    id: res.data.id,
  }
...
export const updateAmbassador = (ambassador: IAmbassador) => (dispatch: any) => {
AmbassadorService().updateAmbassador(ambassador).then((res:any) => {
  const updAmb = {
    ...ambassador,
    id: res.data.id,
  }
